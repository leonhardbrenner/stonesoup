...
class CoreApplication @Inject constructor(val dao: Dao, val seedsService: SeedsService) {
    ...
    fun routesFrom(routing: Routing) = routing.apply {
        routesFromMySeeds()
        routesFromDetailedSeed()
        routesFromSeedCategory()
        routesFromChore()
    }
    ...
    fun Routing.routesFromMySeeds() = route(SeedsDto.MySeeds.path) {
        get {
    ...
    fun Routing.routesFromDetailedSeed() = route(SeedsDto.DetailedSeed.path) {
        get {
            call.respond(seedsService.getDetailedSeeds())
        }
    }

    fun Routing.routesFromSeedCategory() = route(SeedsDto.SeedCategory.path) {
        get {
            call.respond(seedsService.getCategories())
        }
    }
    fun Routing.routesFromChore() = route(SeedsDto.Chore.path) {
        get {
